...
class InSilicoPerturber:
    valid_option_dict = {
        ...
        "model_type": {"Pretrained", "Pretrained_TME",  
                       "GeneClassifier", "CellClassifier","CellClassifier_TME",
                       "MTLCellClassifier", "MTLCellClassifier-Quantized"},
        "num_classes": {int},
    ...
    def __init__(
        ...
        *(TBA)* perturb_rank_shift : None, {1,2,3}
        ...
        genes_to_perturb : "all", list  【扰动模式】"all" 表示全部基因，若给定一个gene list，会将gene list全部一起扰动
        ...
        combos : {0,1}
            | Whether to perturb genes individually (0) or in pairs (1).  【需要genes_to_perturb=="all"】
        anchor_gene : None, str  【需要genes_to_perturb=="all"】
        ...
        emb_mode : {"cls", "cell", "cls_and_gene","cell_and_gene"}
            【如果存在cls token：则设置 "cls"或者"cls_and_gene", 如果不存在，则设置 "cell"或者"cell_and_gene"】
            | Whether to output impact of perturbation on CLS token, cell, and/or gene embeddings.
            【基因Embedding仅支持与初始状态细胞的距离计算，不支持与终点状态的细胞计算】
            | Gene embedding shifts only available as compared to original cell, not comparing to goal state.  【例如OSKM计算】
        cell_emb_style : "mean_pool"
        ...
        state_embs_dict : None, dict  各个状态节点的先验Embedding
        ...
        if self.genes_to_perturb == "all":
            ...
            # 如果设置了一组基因（e.g. OSKM）作为genes_to_perturb，则anchor_gene与combos都将无效
    ...
    def validate_options(self):
        ...
        for attr_name, valid_options in self.valid_option_dict.items():
            # self.__dict__ 保存了对象设置的全部初始属性
            attr_value = self.__dict__[attr_name]
            # 如果属性值不是list或dict，则判断是否在有效选项中
            if type(attr_value) not in {list, dict}:
                ...
                if attr_name in ["anchor_gene"]:
                    if type(attr_name) in {str}:
                        continue
            # 如果有效值仅限制了数据类型（没有具体的候选值），则仅需要验证数据类型
            valid_type = False
    ...
    def perturb_data(
        ...
        **Parameters:**
        ...
        self.max_len = pu.get_model_input_size(model)   # 2048/4096
        # 使用哪一个注意力层输出的embedding
        layer_to_quant = pu.quant_layers(model) + self.emb_layer  # -1 / 0
    ...
    def isp_perturb_set(
        ...
        def make_group_perturbation_batch(example):
            ...
            if self.perturb_type == "delete":
                # 将gene set直接从rank中删除：update example["input_ids"]
                example = pu.delete_indices(example)
            elif self.perturb_type == "overexpress":
                # 将gene set移到rank最前面
                example = pu.overexpress_tokens(
                    example, self.max_len, self.special_token  # None
                )
                example["n_overflow"] = pu.calc_n_overflow(
                    self.max_len,
                    example["length"],
                    self.tokens_to_perturb, # 用户想要OE的总gene list
                    indices_to_perturb,     # 其中有已经在rank中的gene
                )
            return example
        ...
        for i in trange(0, total_batch_length, self.forward_batch_size):
            ...
            if self.cell_emb_style == "mean_pool":
                ...
                # 计算OE前后时，如果OE前，该基因已经在rank中，那么需要删除该基因对应的emb，再pad
                original_emb = pu.remove_perturbed_indices_set(
                    full_original_emb,
                    self.perturb_type,
                    indices_to_perturb,
                    self.tokens_to_perturb,
                    minibatch["length"],  # not use
                )
                # OE后的Embedding
                ...
                if (
                    ...
                    # 计算OE前后的基因相似度（例如paper中OSKM的例子）
                    # 因此需要保证perturbation_emb与original_emb的shape完全一致。这也是上面这么麻烦操作的原因
                    # 因此在上面操作时，不用在意移除index后对cell Embedding的影响，不会用cell level
                    ...
                    # gene_cos_sims.shape:[batch_cells, n_perturbation_genes]
                    # 第一行第一列，表示在第一个细胞中基因1在扰动前后的相似度
                ...
                if self.cell_states_to_model is not None:
                    original_cell_emb = pu.mean_nonpadding_embs(
                        full_original_emb,
                        torch.tensor(minibatch["length"], device=self.device),
                        dim=1,
                    )
                    perturbation_cell_emb = pu.mean_nonpadding_embs(
                        full_perturbation_emb,
                        torch.tensor(perturbation_batch["length"], device=self.device),
                        dim=1,
                    )
            ...
            if self.cell_states_to_model is None:
                ...
                # 计算一个细胞中所有基因的shift的均值，作为该细胞的shift水平
                if self.perturb_type == "overexpress":
                ...
                cos_sims_data = pu.mean_nonpadding_embs(
                    gene_cos_sims, torch.tensor(nonpadding_lens, device=self.device)
                )  # shape: [batch_cells, 1]
                # cos_sims_dict = defaultdict(list)
                ...
                # cos_sims_dict[(perturbed_genes, "cell_emb")] = [shift values of cels]
    ...
    def isp_perturb_set_special(
        ...
        # 基本与上面isp_perturb_set函数内的定义相同
        def make_group_perturbation_batch(example):
            example_input_ids = example["input_ids"]
            example["tokens_to_perturb"] = self.tokens_to_perturb
            # rank index of perturbed tokens
            indices_to_perturb = [
                example_input_ids.index(token) if token in example_input_ids else None
                for token in self.tokens_to_perturb
            ]
            ...
            if self.perturb_type == "delete":
                ...
                # 因为过表达原本不在rank的基因，导致OE后，若干基因由于序列过长而被剔除
                example["n_overflow"] = pu.calc_n_overflow(
                    self.max_len,
                    example["length"],
                    self.tokens_to_perturb,
        ...
        if self.perturb_type == "overexpress":
            ...
            # 提前对由于OE而可能溢出的序列，进行截断
            filtered_input_data = filtered_input_data.map(
                pu.truncate_by_n_overflow_special, num_proc=self.nproc
            )


        # 对于cls_and_gene模式，除了计算细胞水平的扰动，还计算基因水平的扰动变化
        # Gene embedding shifts only available as compared to original cell
        ...
        for i in trange(0, total_batch_length, self.forward_batch_size):
            ...
            # 仅关注没有被直接扰动的基因所受到的embedding变化
                ...
                # remove special tokens (cls / eos) and padding
                ...
                # 保存stored_gene_embs_dict，一个item表示一个基因在多个细胞的扰动影响list
                for cell_i, genes in enumerate(gene_list):
                    for gene_j, affected_gene in enumerate(genes):
                        if len(self.genes_to_perturb) > 1:
                            tokens_to_perturb = tuple(self.tokens_to_perturb)
    ...
    def isp_perturb_all(
        ...
        #  'alt_states': ['hcm']}
        if self.cell_states_to_model is None:
            cos_sims_dict = defaultdict(list)  # gene
        else:
            cos_sims_dict = {
                state: defaultdict(list)
                for state in pu.get_possible_states(self.cell_states_to_model)
        ...
        #  'nf': defaultdict(list, {}),
        #  'hcm': defaultdict(list, {})}

        if self.emb_mode == "cell_and_gene":
            stored_gene_embs_dict = defaultdict(list)

        # 单次扰动一个基因（each all gene）或者两个基因 （each all gene paired with anchor gene）
        ...
        for h in trange(len(filtered_input_data)):
            ...
            # 将扰动基因在gene list移除，方便后续统计gene embedding（非扰动基因的shift）
            perturbed_gene_dict = {
                gene: gene_list[:i] + gene_list[i + 1 :]
                for i, gene in enumerate(gene_list)
            }
            
            # 制作当前细胞每个基因被扰动的datasets 【没有用到gene_list】
            # 每个样本代表一种基因扰动情况
            perturbation_batch, indices_to_perturb = pu.make_perturbation_batch(
                example_cell,
                self.perturb_type,
                self.tokens_to_perturb,
                self.anchor_token,  # for two-gene isp
                self.combos,
                self.nproc,
            )
            ...
            for i in trange(
                ...
                if (
                    ...
                    # gene embedding without perturbed genes
                ...
                if self.cell_states_to_model is not None:  # cell mode
