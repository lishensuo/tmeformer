...
def get_embs(
    ...
    summary_stat=None,
    silent=False,
    **kwargs    # for extra input of model
):
    model_input_size = pu.get_model_input_size(model)  # 最大序列长度 2048 
    total_batch_length = len(filtered_input_data)      # dataset

    # {None, "mean", "median", "exact_mean", "exact_median"}
    # summary_stat=None，表示输出每个细胞的Embedding结果
    # summary_stat!=None，表示输出所有细胞的Embedding的Summary结果
    if summary_stat is None:
        embs_list = []
    elif summary_stat is not None:
        # get # of emb dims
        emb_dims = pu.get_model_emb_dims(model)  # 嵌入维度
        if emb_mode == "cell":
            # initiate tdigests for # of emb dims 为每个维度创建一个tdigest
            embs_tdigests = [TDigest() for _ in range(emb_dims)]
        if emb_mode == "gene":
            # 取所有unique gene id作为gene set，每个基因一个tdigest对象
            gene_set = list(
                {
                    element
    ...
    # total_batch_length：总输入样本量
    # 手动迭代，而非使用dataloader

    # if "tme_cell_embs" in kwargs:
    #     tme_cell_embs = kwargs["tme_cell_embs"] # should be torch tensor, shape like (32, 256, 512)
    # else:
    #     tme_cell_embs = None

    tme_cell_embs = kwargs.get("tme_cell_embs") # should be torch tensor, shape like (32, 256, 512)
    if tme_cell_embs is not None:
        if not isinstance(tme_cell_embs, torch.Tensor):
            tme_cell_embs = torch.tensor(tme_cell_embs)
        if tme_cell_embs.shape[0] != len(filtered_input_data):
            raise  ValueError("tme_cell_embs.shape[0] != len(input_dataset)")

    # loop_range = range if silent else trange
    loop_range = range if silent else (lambda *a, **kw: trange(*a, leave=False, **kw))
    # for i in trange(0, total_batch_length, forward_batch_size, leave=(not silent)):
    for i in loop_range(0, total_batch_length, forward_batch_size):
        max_range = min(i + forward_batch_size, total_batch_length)  #确保最后一个batch的索引位置

        minibatch = filtered_input_data.select([i for i in range(i, max_range)])

        max_len = int(max(minibatch["length"]))
        original_lens = torch.tensor(minibatch["length"], device=model.device)
        ...
        if  hasattr(model.config, "use_tme") and model.config.use_tme:
            ...
            minibatch = tu.modify_tme_dataset(model, minibatch)
            ...
            #     tme_size = 256 # random choose one as not use the two column, just placeholder

            # if "tme_cells" not in minibatch.column_names:
            #     minibatch = minibatch.rename_columns({f"tme_cells{tme_size}" : "tme_cells"})

            # if "tme_types" not in minibatch.column_names:
            #     minibatch = minibatch.rename_columns({f"tme_types{tme_size}" : "tme_types"})

            tme_cells_minibatch = minibatch["tme_cells"]
            tme_types_minibatch = minibatch["tme_types"]
            cell_id_minibatch = minibatch["cell_id"]
            sample_id_minibatch = minibatch["sample_id"]
            ...
            with torch.no_grad():
                outputs = model(
                    ...
                    attention_mask=pu.gen_attention_mask(minibatch, device=model.device),
                    ...
                    tme_cell_embs=tme_cell_embs_minibatch.to(model.device) if tme_cell_embs is not None else None,
        ...
        # 第0个hidden_states：输入嵌入层的隐藏状态
        # 第1个hidden_states：第一个注意力层的隐藏状态
        # print(outputs.keys())

        if isinstance(outputs, dict):
            hidden_states = outputs["hidden_states"]
        else:
            # HuggingFace ModelOutput 类型
            hidden_states = outputs.hidden_states

        # embs_i = outputs.hidden_states[layer_to_quant]
        embs_i = hidden_states[layer_to_quant]

        if emb_mode == "cell":  # 取每个序列有效(original_lens)基因的Embedding均值作为细胞embedding
            if cls_present:
            ...
            if summary_stat is None:
                ...
                # 将每个batch，每个维度的embedding更新到tdigest中
            ...
            if summary_stat is None:
                ...
                for h in trange(len(minibatch)):  # 对批量中每个细胞的迭代处理
                    length_h = minibatch[h]["length"]
                    input_ids_h = minibatch[h]["input_ids"][0:length_h]  # 不考虑pad token的Embedding
                    ...
                    dict_h = dict(zip(input_ids_h, embs_h)) # 每个细胞的有效基因id为key，其Embedding作为value
        ...
        overall_max_len = max(overall_max_len, max_len)  # 最后输出的max len
        del outputs
        del minibatch
    ...
    if summary_stat is None:
        if (emb_mode == "cell") or (emb_mode == "cls"):
            ...
            embs_stack = pu.pad_tensor_list(
                embs_list,
                overall_max_len,  # 所有小批量的最大值（理论上应该是2048/4096）
                pad_token_id,
                model_input_size,
                1,
                pu.pad_3d_tensor,
...
def accumulate_tdigests(embs_tdigests, mean_embs, emb_dims):
    ...
    # TDigest.batch_update比TDigest.update效率低一点
    [
...
def label_gene_embs(embs, downsampled_data, token_gene_dict):
    ...
    for i in range(embs.size()[0]):
        ...
        dict_i = dict(
            zip(
                downsampled_data[i]["input_ids"][0:length],  # e.g. [2, 3] 仅关注非填充的有效基因
                embs[i, :, :].unsqueeze(dim=1),              # e.g. torch.Size([3, 1, 4])
            )
        )
        # {2: tensor([[ 0.9242, -0.7955, -0.3726,  0.0360]]),   【1×4】
        #  3: tensor([[0.4007, 0.7441, 0.6165, 0.7408]])}
...
class EmbExtractor:
    valid_option_dict = {
        "model_type": {"Pretrained", "Pretrained_TME", "GeneClassifier", "CellClassifier", "CellClassifier_TME"},
        "num_classes": {int},
    ...
    def validate_options(self):
        ...
        for attr_name, valid_options in self.valid_option_dict.items():
            # self.__dict__以字典的形式囊括了所有上述__init__定义的属性
    ...
    def extract_embs(
        ...
        **Parameters:**
        ...
        output_format : {"csv", "npy"}
            | Format to save embedding data as.
        ...
        **Examples:**
        ...
        downsampled_data = pu.downsample_and_sort(filtered_input_data, self.max_ncells, sort_by_length=sort_by_length)
        model = pu.load_model(
            self.model_type, self.num_classes, model_directory, mode="eval", device=self.device
        )
        layer_to_quant = pu.quant_layers(model) + self.emb_layer  # 6 + -1 = 5
        embs = get_embs(
            model=model,
        ...
        if output_directory is None and output_prefix is None:
            save_output = False
        if cell_state is None and save_output:
            output_path = (Path(output_directory) / output_prefix).with_suffix(f".{output_format}")
            if output_format == "csv":
                embs_df.to_csv(output_path)
            elif output_format == "npy":
                np.save(output_path, embs_df.values)  # NOTE float32
            else:
                raise ValueError(f"Invalid output_format: {output_format}")

        else:
            print("The embeddings are not saved to local file.")

        # 所谓的exact summary就是指常规的统计，而non-exact summary则是指tdigest的近似统计
    ...
    # 调用get_state_embs时，summary_stat只能是["exact_mean", "exact_median"]之一
    def get_state_embs(
        self,
