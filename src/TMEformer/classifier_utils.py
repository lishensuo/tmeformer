...
def subsample_by_class(labels, N):
    # 使用defaultdict，键可以是任何类型，值是list类型。
...
def validate_and_clean_cols(train_data, eval_data, classifier):
    ...
    cols_to_keep = [label_col] + ["input_ids", "length",'cell_id', 'sample_id', 'tme_cells', 'tme_types']
...
def label_classes(classifier, data, gene_class_dict, nproc):
    if classifier == "cell":
        ...
        def if_contains_label(example):
            ...
            b = example["input_ids"]
            # isdisjoint: 若没有交集，则返回True。而前面的not进一步取反
            # 最终含义： 若细胞非零基因包含任一label genes，则返回True
...
def label_gene_classes(example, class_id_dict, gene_class_dict):
    # 将 gene_class_dict 转换为 {geneA1: Gene_label_A, geneB1: Gene_label_B, ...}
    flat_gene_class_dict = {
        gene: label
        for label, genes in gene_class_dict.items()
        for gene in genes
    }

    # 按照转换后的字典进行处理
    return [
        class_id_dict.get(flat_gene_class_dict.get(token_id, -100), -100)
...
def prep_gene_classifier_all_data(
    ...
    # 阴性标签与阳性标签是以基因为单位
    # 而标签的数量是由表达的细胞来统计，例如有的细胞可能表达多个标签基因，有的细胞仅表达一个标签基因
    # 因此并不是说阴性标签与阳性标签的基因数相同，就是balance，而是要看细胞表达情况
    if balance is True:
...
def balance_gene_split(subset_data, label_dict_subset, num_proc):
    ...
    # 统计每个label类别基因在所有细胞出现的次数
    # label_dict_subset：dict(zip(targets, labels))
        ...
        if label_ratio_0to1 > 10 / 8:
            ...
            for i in range(max_ntrials):
                ...
                label0_nremove = max(
                    ...
                    int(
                        np.floor(
                            label0_ngenes - label0_ngenes / (label_ratio_0to1 * boost) # 100 - 100 / (100*1.1) = 99.1
                        )
                    ),
                )
                random.seed(i)
...
def count_genes_for_balancing(subset_data, label_dict_subset, num_proc):
    def count_targets(example):
        ...
        # 统计每个细胞内的阴性(label-0)与阳性(label-1)基因数量
...
def compute_metrics(pred):
    ...
    # 判断是否为回归任务（根据标签类型）
    if np.issubdtype(labels.dtype, np.floating):  # float 类型 → 回归任务
        # 若是单输出回归，确保维度一致
        if preds.shape[-1] == 1:
            preds = preds.squeeze()
        return {
...
def get_default_train_args(model, classifier, data, output_dir):
    num_layers = pu.quant_layers(model)  # 注意力层数
    freeze_layers = 0
    batch_size = 12
    if classifier == "cell":
        epochs = 10
    ...
    training_args = {
        ...
        "save_strategy": "epoch",
        "save_steps": -1, 
        "save_total_limit": 1,  #不保存checkpoint，以节约空间
...
def load_best_model(directory, model_type, num_classes, mode="eval", device = "cuda:0"):
    file_dict = dict()
    for subdir, dirs, files in os.walk(directory): # 遍历一个目录及其子目录
        # subdir：当前目录
        # dirs：当前目录的子目录
        # files：当前目录的文件
        for file in files:
    ...
    for subdir, dirs, files in os.walk(f"{directory}/{model_superdir}"):
        for file in files:
            if file.endswith("model.safetensors"):
                model = pu.load_model(model_type, num_classes, f"{subdir}", mode, device)
    return model
