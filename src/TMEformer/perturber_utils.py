...
from transformers import (
    ...
    # BertForTokenRegression,
    BitsAndBytesConfig,
)

from .tme.TmeModeling_bert import TmeBertForMaskedLM, TmeBertForSequenceClassification
from .tme.TmeModeling_bert import TmeBertForMultiGeneExpressionPrediction
from .tme.TmeModeling_bert import TmeBertForCellClassification
...
def load_and_filter(filter_data, nproc, input_data_file):
    ...
    elif isinstance(input_data_file, Dataset):
...
# load model to GPU
def load_model(model_type, num_classes=None, model_directory=None, mode=None, quantize=False, device="cuda:0"):
    ...
    if "adapter_model.safetensors" in os.listdir(model_directory):
        print("Detected quantized model. Try to load quantized model.")
        quantize = True
    ...
    if quantize:
        if model_type == "MTLCellClassifier":
            ...
            quantize_config = BitsAndBytesConfig(
                ...
                # load_in_4bit=True,
                # bnb_4bit_quant_type="fp4",              # 相比 nf4 更稳定，且差异不大
                # bnb_4bit_use_double_quant=False,        # 禁用双量化，避免额外精度不确定
                # bnb_4bit_compute_dtype=torch.float16    # 更确定性，bfloat16 虽快但非确定
            ...
            peft_config = LoraConfig(
                lora_alpha=128,  # 缩放因子(对原参数的更新程度：值越大，更新程度越大)
                # lora_alpha=16, 
                lora_dropout=0.1,
                r=64,   # 低秩矩阵的秩(对参数数量的影响：值越大，参数量越多)
                # r=4,
                bias="none",
    ...
    # 加载Qlora微调后的模型
    if mode == "eval" and quantize:
        # base_model_type = "Pretrained_TME"
        with open(f"{model_directory}/adapter_config.json") as f:
        ...
        # TODO
        # model = TmeBertForMaskedLM.from_pretrained(**model_args)
        model = TmeBertForSequenceClassification.from_pretrained(**model_args) ## 应该加载这个模型？？
    ...
    model_classes = {
        ...
        "Pretrained_TME": TmeBertForMaskedLM,
        # "CellClassifier_TME": TmeBertForSequenceClassification,
        "CellClassifier_TME": TmeBertForCellClassification,
        "CellClassifier_TME_MultiEXP": TmeBertForMultiGeneExpressionPrediction
    }
    ...
    if model_type not in  ["Pretrained", "Pretrained_TME"]:
        # 仅适用于微调模型
...
def downsample_and_sort(data, max_ncells, sort_by_length = False): 
    num_cells = len(data)
...
def delete_indices(example):
    ...
    for index in sorted(indices, reverse=True):  #降序操作：表示从索引大（Rank靠后）的基因开始删除，避免列表缩短而导致索引混乱
...
# gene list的oe模式，可以针对基因不在cell中的
def overexpress_tokens(example, max_len, special_token):
    # -100 indicates tokens to overexpress are not present in rank value encoding
    # 
    if example["perturb_index"] != [-100]: # 如果基因本身就在这个序列中，则删除
...
def calc_n_overflow(max_len, example_len, tokens_to_perturb, indices_to_perturb):
    n_to_add = len(tokens_to_perturb) - len(indices_to_perturb) # 有多少OE基因之前不在原有细胞序列中
    ...
    if perturb_type == "overexpress":
        ...
        if num_perturbed == 1:
            ...
            if all(v is [None] for v in indices_to_perturb_orig):
                # 被OE的基因先 前都不在cell的rank中，则Embedding可以直接用
                return emb
        else:
            indices_to_perturb_orig = []
            ...
            for idx_list in indices_to_perturb:
                indices_to_perturb_orig.append(
                    ...
                    # idx_list if idx_list != [-100] else [None]
                )
...
def make_perturbation_batch(
    ...
    if combo_lvl == 0 and tokens_to_perturb == "all":
        ## 对所有基因依次进行isp，常用！！！
        if perturb_type in ["overexpress", "activate"]: #过表达/激活操作，只能对最高排名第二的基因操作
            range_start = 1
        elif perturb_type in ["delete", "inhibit"]:
            range_start = 0
        indices_to_perturb = [
            [i] for i in range(range_start, example_cell["length"][0]) # e.g. [[0],[1],[2]]
        ]  # rank index
    ...
    ## to implement
        ...
        indices_to_perturb = [item for item in indices_to_perturb if item is not None] # 剔除因i==anchor_index而产生的None
        # e.g. [[0, 998], [1, 998], [2, 998]]
    else:
        # 指定一个genelist，常见！！！
        example_input_ids = example_cell["input_ids"][0]
        indices_to_perturb = [
        ...
        indices_to_perturb = [item for item in indices_to_perturb if item is not None] # [[0], [4095]]
    ...
    perturbation_dataset = Dataset.from_dict(
        {
            "input_ids": example_cell["input_ids"] * length,  #每个扰动基因初始化一个样本
            "perturb_index": indices_to_perturb,  #每个被扰动基因在input_ids中的位置，而非该基因的token
        }
    )
...
def pad_xd_tensor(tensor, pad_token_id, max_len, dim):
    ...
    # [0, 0, 0, 5, 0, 0] 表示 对dim=1，填充后侧长度为5
    # [0, 2, 0, 0, 0, 0] 表示 对dim=2，填充后侧长度为2
...
def gen_attention_mask(minibatch_encoding, max_len=None, device="cuda:0"):
    if max_len is None:
...
def mean_nonpadding_embs(embs, original_lens, dim=1):
    ...
    # 广播机制： tensor([0, 1, 2]) < tensor([1, 1, 2]).unsqueeze(1) 【3 × 1】
    ...
    if embs.dim() == 3:
        ...
        # 将pad字符的Embedding替换为0
        masked_embs = embs.masked_fill(~mask.unsqueeze(2), 0.0)

        # compute the mean across the non-padding dimensions
        mean_embs = masked_embs.sum(dim) / original_lens.view(-1, 1).float()  # 后者类似 original_lens.unsqueeze(1)
...
def quant_cos_sims(
    ...
    if cell_states_to_model is None or emb_mode == "gene":
        cos_sims = cos(perturbation_emb, original_emb).to(device)
        ...
        for state in possible_states:
            ...
            cos_sims[state] = cos_sim_shift(
                ...
                perturbation_emb,
                # end
                state_embs_dict[state].to(device),  # required to move to cuda here
                cos,
            )
