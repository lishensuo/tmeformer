...
class AttentionPool(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # attention_weights: (hidden_size, 1)
        # output: (batch_size, sequence_length, 1) 序列中，每个 token 的权重
        attention_scores = torch.matmul(hidden_states, self.attention_weights)
        # (batch_size, sequence_length, 1)： 每个序列所有token权重和为1
        attention_scores = torch.softmax(attention_scores, dim=1)
        # 每个序列，所有token的加权和，得到 每个序列的Embedding表示 (batch_size, hidden_size)
...
class GeneformerMultiTask(nn.Module):
    def __init__(
        ...
        use_attention_pooling=False, #如何表示cell Embedding，False表示使用CLS token
    ):
