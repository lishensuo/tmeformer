        ...
        with open(file_path, "rb") as fp:
            ...
            if cell_states_to_model is None:
                ...
                for state_value in cell_states_list:
                    ...
                    # new_dict  {(9057, 'cell_emb'): [0.0021161437034606934],...}
                    for key in new_dict:
                        try:
...
def get_gene_list(dict_list, mode):
    if mode == "cell":
        position = 0   # 基因token在key元组的第0个位置
    elif mode == "gene":
        position = 1
    gene_set = set()
...
def isp_aggregate_gene_shifts(
    ...
    # cos_data_mean = {
    #     ("TP53", "BRCA2"): [0.85, 0.0408, 3],
    #     ("TP53", "MDM2"): [0.765, 0.015, 2],
    #     ("BRCA1", "ATM"): [0.7, 0.0, 1]
    # }
...
def isp_stats_to_goal_state(
    ...
    if genes_perturbed != "all":
        ...
        for i in trange(cos_sims_df.shape[0]):  # 遍历每个基因
            token = cos_sims_df["Gene"][i]
            goal_end_random_megalist += result_dict[
                cell_states_to_model["goal_state"]
            ].get((token, "cell_emb"), [])  # 获取每个基因在目标状态下的cosine sim
            if alt_end_state_exists is True:
                for alt_state in cell_states_to_model["alt_states"]:
        ...
        for i in trange(cos_sims_df.shape[0]):
            ...
            ).pvalue  # 秩和检验：该基因的扰动shift与全部基因扰动shitf的分布差异
    ...
    def get_stats(
        ...
        **Outputs:**
        ...
        if (self.combos == 0) and (self.anchor_token is not None):
            ...
            gene_list = get_gene_list(dict_list, "cell")  # 获取所有的基因

        # initiate results dataframe 初始化表格
