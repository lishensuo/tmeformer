...
class PaddingStrategy(ExplicitEnum):
    ...
    LONGEST = "longest"        # 取批次中最长的序列
    MAX_LENGTH = "max_length"  # 设置固定的最长序列
...
# SpecialTokensMixin 用于处理特殊的token的类
# 相当于定义了有一个tokenizer
class GeneformerPreCollator(SpecialTokensMixin):
    ...
    # 添加这个方法来兼容 Trainer 的保存逻辑
    def save_pretrained(self, output_dir):
        """
        Geneformer 使用基因字典而不是传统的 tokenizer，
        所以这里不需要保存任何内容
        """
        pass
    # token id → token字符
...
class GeneformerPretrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        if example_lengths_file is None:
            raise ValueError(
                "example_lengths_file_file is required; provide either a path to a pickle file "
                "or a list of example lengths."
            )

        # case 1: path to pickle file
        if isinstance(example_lengths_file, str):
            with open(example_lengths_file, "rb") as f:
                self.example_lengths = pickle.load(f)

        # case 2: directly provided list
        elif isinstance(example_lengths_file, list):
            self.example_lengths = example_lengths_file

        else:
            raise TypeError(
                "example_lengths_file must be either a file path (str/Path) or a list of lengths."
            )

        self.eval_example_lengths = kwargs.pop("eval_example_lengths", None)
        
        super().__init__(*args, **kwargs)


    # 修改方法签名：添加 dataset 参数
    def _get_train_sampler(self, dataset=None) -> Optional[torch.utils.data.sampler.Sampler]:
        # 如果没有传入 dataset，使用 self.train_dataset
        if dataset is None:
        ...
        if self.args.group_by_length:
            ...
            return LengthGroupedSampler(
                    dataset=dataset,  # 使用参数而不是 self.train_dataset
                    batch_size=self.args.train_batch_size,
                    lengths=lengths,                    # 每个样本的长度
            ...
            if _is_torch_generator_available:
                return RandomSampler(dataset, generator=generator)  # 使用参数
            return RandomSampler(dataset)  # 使用参数
    
    # For validation evaludation
    def _get_eval_sampler(self, eval_dataset: Dataset) -> Optional[torch.utils.data.sampler.Sampler]:
        """
        ✅ 新增：验证集采样器
        关键：如果验证集也需要 group_by_length，需要使用验证集的 lengths
        """
        ...
        if self.args.group_by_length:
            if is_datasets_available() and isinstance(eval_dataset, Dataset):
                # ⚠️ 关键：使用验证集的 lengths
                lengths = self.eval_example_lengths if self.eval_example_lengths else None
            else:
                lengths = None
