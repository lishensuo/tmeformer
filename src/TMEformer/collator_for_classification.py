...
class PrecollatorForGeneAndCellClassification(SpecialTokensMixin):
    ...
    def save_pretrained(self, output_dir):
        """Dummy method to prevent save errors. Data collator has no state to save."""
        pass
    ...
    def pad(
        ...
        # From list of dicts(samples) to one merged dict(batch)
        ...
        '''
        对于如下单个样本，required_input[0]为整数，而非list
        encoded_inputs = {
            "input_ids": [101, 102],   # 仅包含一个样本
            "attention_mask": [1, 1],
        }
        
        对于如下批量样本，required_input[0]为list
        encoded_inputs = {
            "input_ids": [[101, 102], [103, 104], [105, 106]],  # 三个样本的 input_ids
            "attention_mask": [[1, 1], [1, 1], [1, 1]],          # 三个样本的 attention_mask
        }
        '''
        required_input = encoded_inputs[self.model_input_names[0]]  # self.model_input_names[0]： "input_ids"
...
class DataCollatorForGeneClassification(DataCollatorForTokenClassification):
    ...
    def _prepare_batch(self, features):
        ...
        # "input_ids"/"attention_mask",对于基因会有"labels"
        return batch

    def __call__(self, features):
        batch = self._prepare_batch(features)
        # [Note] When for regression task, labels is a list of floats, should not be converted to int types.
        # batch = {k: torch.tensor(v, dtype=torch.int64) if k != "labels" else v for k, v in batch.items()}
        ...
        for k, v in batch.items():
            if k == "labels":
                ...
                new_batch[k] = v.clone().detach().to(dtype=torch.int64)
            else:
...
class DataCollatorForCellClassification(DataCollatorForGeneClassification):
    ...
    def _prepare_batch(self, features):
        ...
        first = features[0]
        # 细胞分类任务的labels需要特殊处理（它在datasets的label列）
        ...
        # print("Test DataCollatorForGeneClassification", batch["labels"])
