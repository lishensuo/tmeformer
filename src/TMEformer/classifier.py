...
class Classifier:
    ...
    def __init__(
        ...
        **Parameters:**
        ...
        if self.classifier == "cell":
            self.model_type = "CellClassifier_TME"
        ...
        self.ray_config = ray_config    # 超参数寻参
    ...
    def prepare_data(
        ...
        **Parameters**
        ...
        # 注意：对于gene classifier, 暂时还没有添加labels数据
    ...
    def validate(
        ...
        **Parameters**
        ...
        results = []
        all_conf_mat = np.zeros((num_classes, num_classes))  # 混淆矩阵
        iteration_num = 1
        if self.classifier == "cell":
            ...
            targets = pu.flatten_list(self.gene_class_dict.values())  # token
            labels = pu.flatten_list(
                [
                    [class_id_dict[label]] * len(targets)
                    for label, targets in self.gene_class_dict.items()
            ...
            assert len(targets) == len(labels)
            n_splits = int(1 / (1 - self.train_size)) # train_size = 0.8, n_splits = 5
            skf = cu.StratifiedKFold3(n_splits=n_splits, random_state=0, shuffle=True)
            # (Cross-)validate
            test_ratio = self.oos_test_size / (self.eval_size + self.oos_test_size) # 0.1/(0.1+0.1)=0.5
            # 在每一折中，进一步将测试集分为两部分，一部分作为验证集，一部分作为测试集
            # 注意：这里所谓的训练集/验证集/测试集，都是指的基因为单位，而非细胞为单位
            for train_index, eval_index, test_index in tqdm(
                ...
                # 首先根据train_index基因，筛选包含相关基因的细胞，然后为其dataset加入labels列，仅标注其关注的基因。其余非label基因或者eval基因都会被标记为-100
    ...
    def hyperopt_classifier(
        ...
        **Parameters**
        ...
        #         {f"tme_cells{tme_size}" : "tme_cells", f"tme_types{tme_size}" : "tme_types"}
        #     )
        #     eval_data = eval_data.rename_columns(
        #         {f"tme_cells{tme_size}" : "tme_cells", f"tme_types{tme_size}" : "tme_types"}
        #     )
        ...
        trainer.hyperparameter_search(
            ...
            # resources_per_trial={"cpu": 40, "gpu": 0},
        ...
        # 结束后，trainer 会默认加载超参数调优过程中性能最好的模型。
    ...
    def train_classifier(
        ...
        **Parameters**
        ...
        def_training_args["logging_steps"] = logging_steps
        def_training_args["output_dir"] = output_directory
        # eval_data为None时，为最后的Train_all阶段，即获得最终的模型
        # 此时没有eval data，也就无法load_best_model_at_end（根据eval指标，选择保存最佳的checkpoint为最终模型）
        if eval_data is None:
        ...
        trainer = Trainer(
            ...
            train_dataset=train_data,
            eval_dataset=eval_data, 
            compute_metrics=cu.compute_metrics,  # Acc/F1 metrics
        )
